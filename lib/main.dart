...
import 'package:machine_test/template/Card.dart';
...
class GithubAPIState extends State<GithubAPI> with TickerProviderStateMixin {
  ...
  Future _getUser(String text) async{
    ...
    String url = "https://api.github.com/users/"+text;
      ...
      if(resBody['avatar_url'] !=  null) {
        ...
        loc.setProfileName(resBody['name']);
        loc.setImageUrl(resBody['avatar_url']);
  ...
  Widget build(BuildContext context) {
    return new Scaffold(
      appBar: new AppBar(
          title: new Text("Github API Integration",style: TextStyle(color: Colors.black),),
